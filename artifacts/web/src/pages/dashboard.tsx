...
import { useGetDashboardSummary, useGetLocations } from "@workspace/api-client-react";
import type { GetDashboardSummaryParams } from "@workspace/api-client-react";
...
export default function Dashboard() {
  const [period, setPeriod] = useState<GetDashboardSummaryParams["period"]>("today");
  const [locationId, setLocationId] = useState<string>("all");

  const { data: locations } = useGetLocations();

  const params: GetDashboardSummaryParams = {
    period,
    locationId: locationId !== "all" ? parseInt(locationId) : undefined,
  };

  const { data: summary, isLoading } = useGetDashboardSummary(params, {
    query: { queryKey: ["dashboard-summary", params] },
  });
  ...
  const periodLabel = PERIOD_LABELS[period ?? "today"] ?? "Period";
    ...
    <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Select value={period ?? "today"} onValueChange={(v) => setPeriod(v as GetDashboardSummaryParams["period"])}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Orders {periodLabel}</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Sales {periodLabel}</CardTitle>
        ...
        <Card className="col-span-3">
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div
                className="flex flex-col items-center justify-center gap-2 rounded-lg border p-4 text-center hover:bg-muted/50 transition-colors cursor-pointer"
                data-testid="action-new-order"
              >
              ...
              <div
                className="flex flex-col items-center justify-center gap-2 rounded-lg border p-4 text-center hover:bg-muted/50 transition-colors cursor-pointer"
                data-testid="action-new-employee"
              >
